import { DayDetails, FullDateComponents, ElementsTask,  TaskData } from "@/types/calendar";

export const days = ["DO", "LU", "MA", "MI", "JU", "VI", "SA"];

export const generateDaysMonths = (actualYear: number): number[] => {
...
export const calculateHeightPorcent = (componentsDateStart: FullDateComponents, componentsDateEnd: FullDateComponents, mdScreen: boolean) => {
  const hourStart = componentsDateStart.hour
  const hourEnd = (componentsDateEnd.amPm == "am" && componentsDateEnd.hour == 0) ? 24 : componentsDateEnd.hour
  const totalHoursEnd = hourEnd + (componentsDateEnd.minutes/60)
  const totalHoursStart = hourStart + (componentsDateStart.minutes/60)
  const heighPorcent = (totalHoursEnd > totalHoursStart) ? (mdScreen ? 144 : 96) * ((totalHoursEnd - totalHoursStart) / 24) : (mdScreen ? 144 : 96) * ((totalHoursStart - totalHoursEnd) / 24)
  return heighPorcent;
}
...
export const arrSimpleDaysWeek = (day: number, dayFinal: number): number[] => {
  const arrayDays = [];
  ...
  if (day < dayFinal) {
    ...
    for (let i = day; i < daysMonth; i++) {
      // Suponiendo 31 días como límite máximo
      arrayDays.push(i);
    }
...
export const filterTaskByWeek = (
  ...
  const tasksWeek = tasks.filter((task) => {
    ...
    const conditionYear =
      dateStart.year === year || dateStart.year === yearFinal;
    const conditionMonth =
      dateStart.month === month || dateStart.month === monthFinal;
    ...
    const newCondition =
      dateStart.day <= 7 ? dateStart.month === monthFinal : true;
    const conditionDay =
      arrDays.includes(dateStart.day) &&
      !(dateStart.day <= 7 && dateStart.month < monthFinal) &&
      subDay &&
      newCondition;
