import { DateComponents, DateSimpleProps, TaskData } from "@/types/calendar";
import { getComponentsFullDate, generateDaysMonths} from "./calendar";


const getHoursDay = (date: string) => {
    const hours = date.substring(11, 17);  
    return hours;
};

const getComponentsDate = (date: string) => {
...
const generateDatesIntervalTask = (dateTaskS: string, dateTaskE: string, dateInterval: string) => {
    const datesInterval = [];
...
export const createTaskDaily = (data:TaskData): DateSimpleProps[] => {
    ...
    const tasks: DateSimpleProps[] = [];
    delete data.dateRepeat;
    for (let index = 0; index < datesInterval.length; index++) {
        const newTask: DateSimpleProps = { ...data, dateStart: datesInterval[index][0], dateEnd: datesInterval[index][1] };
        tasks.push(newTask);
    }
    return tasks;
};
...
const generateWeekIntervalsDates = (dateTaskS: string, dateTaskE: string, dateInterval: string) => {
    const componentsDateInterval = getComponentsDate(dateInterval);
    const componentsDateTaskS = getComponentsFullDate(dateTaskS);
    const hoursStart = getHoursDay(dateTaskS);
    const hoursEnd = getHoursDay(dateTaskE);
...
const validateWeekIntervalDAtes = (arr: string[][], dateInterval: string): string[][] => {
    const componentsDateInterval: DateComponents = getComponentsDate(dateInterval);
    for (let index = arr.length; index > 0; index--) {
        const componentsDate: DateComponents = getComponentsFullDate(arr[index-1][1]);
...
const generateMonthIntervalsDates = (dateTaskS: string, dateTaskE: string, dateInterval: string) => {
